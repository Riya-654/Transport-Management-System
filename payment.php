...
<?php
    ...
    {
        $query1 = "select Seats from drivers where Route_No='$rr'";
        $r1 = mysqli_query($con, $query1);
        $data2 = mysqli_fetch_assoc($r1);
        $seat=$data2['Seats'];
        $seat=$seat-1;

        $query2 = "update drivers set Seats='$seat' where Route_No='$rr'";
        mysqli_query($con, $query2);

        #echo '<script>alert("Done")</script>';
        header("Location: exitP.php");
    }
...
<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
    ...
    <input type="submit" class="button" value="CONFIRM AND PAY NOW" style = "font-size:20px" />
